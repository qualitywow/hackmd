...
在初始化階段，每個UT以隨機方式選擇cadidate SAT進行HO，這稱為exploration period(探索期)。
根據在探索期的行動和觀察，每個UT根據cadidate SAT的Q-value建立一個優先級列表。
在穩定狀態下，UT根據優先級列表選擇cadidate SAT進行HO。
...
![Image](https://i.imgur.com/XKKOySV.png)
圖13：不同UT速度的HO失敗率。
...
- 特別地，QIH透過預測中cadidate SAT的可用channel，可以避免由壅塞引起的HO失敗。
=> QIH在六種算法中的HO失敗率最低。
